...
function ENT:Initialize()
    local cfg = UniversalAmmo.Config.box
...
function ENT:OnTakeDamage(dmg)
    if UniversalAmmo.Config.box.maxHealth == 0 then return end
...
function ENT:Use(_, caller)
    ...
    if not UniversalAmmo.Config.box.enabled then return end
    ...
    local amount = UniversalAmmo.GetAmmoAmount(ammoName, wep, "box")
    caller:GiveAmmo(amount, ammoName, false)
    
    local altAmount = UniversalAmmo.GetAmmoAmount(altAmmoName, wep, "box") / 7
    if altAmmoName then
