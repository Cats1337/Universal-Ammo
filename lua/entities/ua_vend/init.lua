...
function ENT:Initialize()
    local cfg = UniversalAmmo.Config.vend
    ...
    self:SetMaterial("models/ua_vend")
...
function ENT:OnTakeDamage(dmg)
    if UniversalAmmo.Config.vend.maxHealth == 0 then return end
...
function ENT:Use(_, caller)
    ...
    if not UniversalAmmo.Config.vend.enabled then return end
    ...
    local amount = UniversalAmmo.GetAmmoAmount(ammoName, wep, "vend")
    caller:GiveAmmo(amount, ammoName, false)
    
    local altAmount = UniversalAmmo.GetAmmoAmount(altAmmoName, wep, "vend") / 7
    if altAmmoName then
